"""German conversion validator.

Parity check between the reference German TTSModel and the
CoreML-composed German pipeline. This is NOT part of the pytest suite
because it requires:
  - Network access at first run (voice + weights download)
  - Downloaded German voices in `voices_german/`
  - Pre-converted `Artifacts/de_fp16/` bundle

Usage:
    .venv/bin/python -m pockettts_coreml.oracle.verify_german \\
        [--voice voices_german/alba.safetensors] \\
        [--artifacts Artifacts/de_fp16] \\
        [--prompt "Pocket TTS ist ..."] \\
        [--seed 42]
...
Exits non-zero if any gate fails. Writes `/tmp/de_reference.wav` and
`/tmp/de_coreml.wav` for manual listen-compare.
"""
...
LOGGER = logging.getLogger("pockettts_coreml.oracle.verify_german")
...
DEFAULT_PROMPT = "Pocket TTS ist ein leichtgewichtiges Text-zu-Sprache-Modell."
DEFAULT_VOICE = REPO_ROOT / "voices_german" / "alba.safetensors"
DEFAULT_ARTIFACTS = REPO_ROOT / "Artifacts" / "de_fp16"
LANGUAGE = "german"
SAMPLE_RATE = 24000
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="verify_german")
    ...
    _write_wav(Path("/tmp/de_reference.wav"), ref)
    _write_wav(Path("/tmp/de_coreml.wav"), tst)
