"""Italian conversion validator.

Parity check between the reference Italian TTSModel and the
CoreML-composed Italian pipeline. This is NOT part of the pytest suite
because it requires:
  - Network access at first run (voice + weights download)
  - Downloaded Italian voices in `voices_italian/`
  - Pre-converted `Artifacts/it_fp16/` bundle

Usage:
    .venv/bin/python -m pockettts_coreml.oracle.verify_italian \\
        [--voice voices_italian/alba.safetensors] \\
        [--artifacts Artifacts/it_fp16] \\
        [--prompt "Pocket TTS è ..."] \\
        [--seed 42]
...
Exits non-zero if any gate fails. Writes `/tmp/it_reference.wav` and
`/tmp/it_coreml.wav` for manual listen-compare.
"""
...
LOGGER = logging.getLogger("pockettts_coreml.oracle.verify_italian")
...
DEFAULT_PROMPT = "Pocket TTS è un modello leggero di sintesi vocale."
DEFAULT_VOICE = REPO_ROOT / "voices_italian" / "alba.safetensors"
DEFAULT_ARTIFACTS = REPO_ROOT / "Artifacts" / "it_fp16"
LANGUAGE = "italian"
SAMPLE_RATE = 24000
FRAME_SIZE = 1920
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="verify_italian")
    ...
    _write_wav(Path("/tmp/it_reference.wav"), ref)
    _write_wav(Path("/tmp/it_coreml.wav"), tst)
