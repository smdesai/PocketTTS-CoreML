...
public final class Tokenizer: @unchecked Sendable {
    ...
    public init(modelURL: URL) throws {
        ...
        guard let p = path.withCString({ sentencepiece_create($0) }) else {
            throw TokenizerError.modelLoadFailed(path: path)
        }
        self.proc = p
        self.vocabSize = Int(sentencepiece_get_piece_size(p))
    }
    ...
    public func encode(_ text: String) -> [Int32] {
        ...
        let n = withUnsafeMutablePointer(to: &idsPtr) { outer -> Int32 in
            text.withCString { cstr in
                sentencepiece_encode_as_ids(proc, cstr, outer)
            }
        }
        guard n > 0, let ids = idsPtr else { return [] }
        defer { sentencepiece_free_ids(ids) }
    ...
    public func decode(_ ids: [Int32]) -> String {
        ...
        return ids.withUnsafeBufferPointer { buf -> String in
            guard let cstr = sentencepiece_decode_ids(proc, buf.baseAddress, Int32(buf.count)) else {
                return ""
            }
            defer { sentencepiece_free_string(cstr) }
    ...
    public func pieces(for text: String) -> [String] {
        ...
        let n = withUnsafeMutablePointer(to: &out) { outer -> Int32 in
            text.withCString { cstr in
                sentencepiece_encode_as_pieces(proc, cstr, outer)
            }
        }
        guard n > 0, let pieces = out else { return [] }
        defer { sentencepiece_free_pieces(pieces, n) }
        var result: [String] = []
        for i in 0 ..< Int(n) {
